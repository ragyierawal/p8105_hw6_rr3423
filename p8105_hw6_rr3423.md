...
cv_df = 
  cv_df %>% 
  mutate(
    lm_1 = map(train, ~lm(bwt ~ gaweeks + momage + mrace + wtgain, data = .x)),
    lm_2 = map(train, ~lm(bwt ~ blength + gaweeks, data = .x)),
    lm_3 = map(train, ~lm(bwt ~ bhead*blength*babysex, data = .x))
  ) %>% 
  mutate(
    rmse_lm_1 = map2_dbl(lm_1, test, ~rmse(model = .x, data = .y)),
    rmse_lm_2 = map2_dbl(lm_2, test, ~rmse(model = .x, data = .y)),
    rmse_lm_3 = map2_dbl(lm_3, test, ~rmse(model = .x, data = .y))
  )
```
