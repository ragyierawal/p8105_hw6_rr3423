...
variance (heteroscedasticity). In the ideal situation, I would like to
...
pretty evenly distributed around 0.

However, I will still try to further improve my proposed regression
model. I will do this by only including the factors that were found to
be significant from this regression model (gaweeks, momage, mrace,
wtgain).

### Improving proposed regression model

I will try to improve my proposed regression model by only including the
significant factors, which involves removing the fincome factor:
...
significant because they have a p-value less than 0.05 in the linear
regression model output: gaweeks, momage, mraceBlack, mracePuerto Rican,
and wtgain. The mracePuerto Rican factor is not significant because the
p-value is greater that 0.05, but not as much as it was in my original
