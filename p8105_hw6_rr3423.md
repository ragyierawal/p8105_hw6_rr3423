...
complex with too many variables include, which could reduce its
prediction accuracy. Model 2 might be too simple, which could also
reduce its prediction accuracy.

Based on this cross validation, it appears that head circumference, head
length, and baby sex are significant predictors for birthweight.
